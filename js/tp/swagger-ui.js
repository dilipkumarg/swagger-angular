...
var Docs = {
	...
	shebang: function() {
        ...
        fragments.shift(); // get rid of api name.
...
//                log("li_content_dom_id " + li_content_dom_id);
  ...
  toggleOperationsForResource: function(resource) {
    ...
    if(Docs.isOperationsExpandedForResource(resource)) {
  ...
  isOperationsExpandedForResource: function(resource) {
    var operations = $('li#resource_' + Docs.escapeResourceName(resource) + ' li.operation div.content');
    var i = 0;
    for(i = 0 ; i < operations.length; i++) {
      if($(operations[i]).is(":visible") === true) {
        return true;
      }
    }
    return false;
...
(function() {
  ...
  SwaggerUi = (function(_super) {
    ...
    SwaggerUi.prototype.api_name = "default";
  ...
  OperationView = (function(_super) {
    ...
    OperationView.prototype.showErrorStatus = function(data, parent) {
        if (data.status === 401) {
            var response = confirm("Your need to login to proceed..!! Press OK to redirect login page?");
            if (response === true) {
                console.log("redirecting to login page..");
                window.location.assign("/login?target_url=" + encodeURIComponent(window.location.href));
            }
            // hiding the throbber image
    ...
    OperationView.prototype.formatXml = function(xml) {
      ...
      reg = /(>)(<)(\/*)/g; /*sample comment to ensure highlighting*/
      wsexp = /[ ]*(.*)[ ]+\n/g;
      contexp = /(<.+>)(.+\n)/g;
